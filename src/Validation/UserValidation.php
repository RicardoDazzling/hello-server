...
final class UserValidation implements Validate
{
    ...
    {
        // validation schema
        if (!array_key_exists('name', $data) && $mandatory) throw new ValidationException('Name is empty.');
        ...
        if (!array_key_exists('email', $data) && $mandatory) throw new ValidationException('Email is empty.');
        else if (!v::email()->validate($data['email'])) throw new ValidationException('Invalid Email.');
